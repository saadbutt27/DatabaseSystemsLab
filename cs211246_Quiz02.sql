...
select D_Id, D_Name, No_of_Employees from (
select e.department_id as D_Id, d.department_name as D_Name, count(*) as No_of_Employees from employees e
join departments d on d.department_id = e.employee_id
group by e.department_id
) temp
order by No_of_Employees,D_Name desc;


-- 2
select concat(first_name, '', last_name) as Employee_Name, salary as Salary,department_id from employees
where salary in (
select salary from employees group by department_id order by salary);
