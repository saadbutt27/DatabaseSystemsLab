...
select productname, unit from products where Price = (select max(Price) from products);

select * from orders where CustomerID in (select customerID from customers where country in ('Germany', 'France', 'Italy'));